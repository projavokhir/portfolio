...
public function getAllSortByIdUpLimit($table_name, $limit){
	return $this->select($table_name, array("*"), "", $limit, "id", true);
}
...
public function getSortByField($table_name, $where, $field, $limit){
	return $this->select($table_name, array("*"), $where, $limit, "id", true);
}
...
public function search($table_name, $fields, $query){
	if(count($fields) == 0) return false;
	$where = "";
	for($i = 0; $i < count($fields); $i++){
		$where .= "`".$fields[$i]."` LIKE '%".$query."%' OR ";
	}

	$where = substr($where, 0, -3);
	$res = $this->select($table_name, $fields, $where, 0, "id", true);
	return $res;
}
